...
const uint16_t ntc_lookup_tab[NTC_TEMP_SCALE] = {
    193,  204,  215,  227,  239,  252,  265,  279,  294,  309,  324,  340,  357,  375,  393,  411,  431,  451,//-40℃开始,4096-202/(202+10)*4096
    ...
    3945, 3949, 3952, 3954, 3957, 3960, 3963, 3966, 3968, 3971, 3973};																				//150℃结束,4096-0.3104/(0.3104+10)*4096
//4.7K
//const uint16_t ntc_lookup_tab[NTC_TEMP_SCALE] = {
...
//	    3788, 3794, 3799, 3805, 3811, 3816, 3822, 3827, 3832, 3837, 3842};																				//150℃结束,4096-0.3104/(0.3104+4.7)*4096 
//100k
...
//	    37 ,  39 ,  42 ,  44 ,  47 ,  49 ,  52 ,  55 ,  58 ,  62 ,  65 ,  69 ,  
			73 ,  77 ,  81 ,  85 ,  90 ,  94 ,
	    99 ,  105 , 110 , 116 , 122 , 128 , 134 , 141 , 148 , 155 , 163 , 170 , 179 , 187 , 196 , 205 , 214 , 224 , 
	    234 , 245 , 256 , 267 , 279 , 291 , 303 , 316 , 330 , 344 , 358 , 372 , 387 , 403 , 419 , 436 , 452 , 470 ,
	    488 , 506 , 525 , 544 , 564 , 585 , 605 , 627 , 648 , 671 , 694 , 717 , 741 , 765 , 789 , 815 , 840 , 866 ,
			893 , 920 , 947 , 975 , 1003 ,1031 ,1060 ,1090 ,1119 ,1149 ,1180 ,1210 ,1241 ,1273 ,1304 ,1336 ,1368 ,1400 ,
	    1433 ,1465 ,1498 ,1531 ,1564 ,1597 ,1630 ,1663 ,1697 ,1730 ,1763 ,1797, 1830 ,1863 ,1896 ,1930 ,1963 ,1996 ,
	    2028 ,2061 ,2094 ,2126 ,2158 ,2190 ,2222 ,2253 ,2284 ,2315 ,2346 ,2376 ,2407 ,2436 ,2466 ,2495 ,2524 ,2553,
	    2581 ,2609 ,2636 ,2664 ,2690 ,2717 ,2743 ,2769 ,2794 ,2819 ,2844 ,2868 ,2892 ,2916 ,2939 ,2962 ,2984 ,3006 ,
			3028 ,3049 ,3070 ,3090 ,3110 ,3130 ,3150 ,3169 ,3188 ,3206 ,3224 ,3242 ,3259 ,3276 ,3293 ,3309 ,3325 ,3341 ,
			3356 ,3372 ,3386 ,3401 ,3415 ,3429 ,3443 ,3456 ,3469 ,3482 ,3494, 3494 ,3507 ,3519 ,3531 ,3542 ,3553 ,3564 ,
			3575 ,3586 ,3596 ,3606 ,3616 ,3626 ,3635 ,3645 ,3654 ,3663 ,3671 ,3680 ,3688 ,3696 ,3704 ,3712 ,3720 ,3727 ,
			3734 ,3742 ,3749 ,3755 ,3762 };  
enum
{
    R22,
    R134A,
...
//    ain_mask_bitmap             = 0x3FF;
    ...
    for (i = AI_NTC1; i < AI_NTC_MAX_CNT; i++)
    {
        g_sVariable.status.u16AI[i] = (((ain_mask_bitmap >> i) & 0x0001) != 0)
                                  ...
                                  : ABNORMAL_VALUE;
//        g_sVariable.status.u16AI[i] = (((ain_mask_bitmap >> i) & 0x0001) != 0)
//                                  ? calc_ntc(ADC1ConvertedValue[i], 0,i)
//                                  : ABNORMAL_VALUE;
    }

//    g_sVariable.status.u16Test[6]=ADC1ConvertedValue[6];
//    g_sVariable.status.u16Test[7]=ADC1ConvertedValue[7];
//    g_sVariable.status.u16Test[8]=ADC1ConvertedValue[8];
				//UV电压
		for (i = AI_LEDUV1_V; i <=AI_LEDUV3_V; i++)
    {
        g_sVariable.status.u16AI[i] = (((ain_mask_bitmap >> (i)) & 0x0001) != 0) ? Calc_UV_ai(ADC1ConvertedValue[i],0,0) : 0;
    }	
//	//滤波
//	for (i = AI_NTC1; i < AI_NTC_MAX_CNT; i++)
//	{
//			g_sVariable.status.u16AI[i] = AVGfilter2(i, g_sVariable.status.u16AI[i]);
//			if(g_sVariable.status.u16AI[i]!= ABNORMAL_VALUE)
//			{
//				sum +=(int16_t) g_sVariable.status.u16AI[i];
//				index++;					
//			}
//	}
