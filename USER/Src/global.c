...
{	// id			 						mapped registers									                min			max					default				       permission	   r/w     chk_ptr
		...
		{	46,			 					&g_sVariable.gPara.CP.Defrost.u16Start_Defrost_Temp,0,		0xFFFF,			-30,						                  1,      NULL},
...
//		{	69,			 					&g_sVariable.gPara.Water.u16TDS_Cail[1],					0,			0xFFFF,			0,						                  1,      NULL},
...
int16_t eeprom_compare_read(uint16_t reg_offset_addr, uint16_t *rd_data)
{
		...
		Tempbuf[0]=I2C_EE_BufRead((uint8_t *)&rd_buf0,CONF_REG_EE1_ADDR+(reg_offset_addr<<1),2);			//从用户区的三处读数据
